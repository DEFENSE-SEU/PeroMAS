...
def _assign_features(matrices, elem_info, formulae, sum_feat=False):
    ...
    for h in tqdm.tqdm(range(len(formulae)), desc='Assigning Features...' , disable=True): # revised by Ryo Fukasawa
...
def generate_features(df, elem_prop='oliynyk',
    ...
    for index in tqdm.tqdm(df.index.values, desc='Processing Input Data', disable=True): # revised by Ryo Fukasawa
    ...
    # print('\tfeaturizing compositions...'.title()) # comment out by Ryo Fukasawa
    ...
    # print('\tcreating pandas objects...'.title()) # comment out by Ryo Fukasawa
    ...
    # get the column names (convert to list for pandas 3.0 compatibility)
