from train import *
from process import *
# settings 由 train_models.py 动态传入参数，不再在此直接导入
# import settings2_train as settings
import datetime
import joblib
import numpy as np
try:
...
def main(
    ...
    # ---------------- HYPEROPT ----------------
        ...
        if model_name == "RF":
            ...
            mlp = MLPRegressor(
                hidden_layer_sizes=tuple([study.best_params["dim"] for _ in range(study.best_params["n_mid"])]),
...
# 注意：现在通过 train_models.py 调用，不再直接运行此文件
# 如需直接运行，请使用: python train_models.py --mode comp_only --model RF
