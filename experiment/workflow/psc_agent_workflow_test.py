...
Analyze the interface quality: compare the defect density of perovskite grown on low-temp ETLs versus high-temp sintered TiO2."""
    ...
    # === Group 5: Frontier Exploration (Q013-Q016) ===
    {
        "id": "Q013",
        "type": "frontier",
        "query": """I heard some perovskites can self-heal after moisture degradation. I want to exploit this for long-life batteries.

Search for additives (like methylamine gas cycles or specific ligands) that promote self-healing/recrystallization.
Design a material system with this self-healing capability clearly defined.
Predict the recovery percentage of PCE after a degradation-healing cycle.
...
Analyze the surface defects: how does the new ligand passivation reduce the trap density on the QD surface?"""
    ...
    # === Group 6: Special Environments & Theory (Q017-Q020) ===
    {
        ...
        "query": """We are designing for IoT sensors under indoor LED light (1000 lux). The spectrum matches wide bandgaps better.

Search for compositions with an optimal bandgap (~1.9 eV) for indoor light harvesting.
Design a Br-rich perovskite recipe specifically for this application.
Predict the output power density (uW/cm²) and Open Circuit Voltage.
Analyze the loss mechanisms: why is the shunt resistance (Rsh) much more critical under low light than under sunlight?"""
    },
    {
        "id": "Q018",
        "type": "special_environment",
        "query": """We are evaluating perovskites for space satellites. They must withstand high-energy proton radiation.

Search for literature on the radiation hardness of perovskites vs silicon.
Design a device structure (specifically the encapsulation and transport layers) to resist radiation damage.
Predict the degradation rate of PCE after a specific radiation dose.
Analyze the damage mechanism: does radiation cause amorphization or create specific point defects like Iodine Frenkel pairs?"""
    },
    {
        "id": "Q019",
        "type": "special_environment",
        "query": """I have a library of 50 new organic amines. I want to use Machine Learning to screen them for 2D passivation.

Search for molecular descriptors (like steric hindrance, hydrogen bonding energy) that correlate with stability.
Instruct the system to screen these 50 amines (hypothetically) based on these descriptors.
Predict the top 3 candidates that will yield the highest phase stability.
Analyze the feature importance: which descriptor is the dominant factor for prediction in your model?"""
    },
    {
        "id": "Q020",
        "type": "special_environment",
        "query": """The Interconnecting Layer (ICL) is the weak point in our all-perovskite tandem.

Search for optimal ICL stacks involving ALD-SnO2 and thin metals (Au/Ag).
Design a detailed layer-by-layer deposition recipe for the ICL to ensure ohmic contact.
Predict the optical transparency and voltage loss across the ICL.
Analyze the recombination: explain how electrons from the bottom cell recombine with holes from the top cell at this interface without loss."""
    },
]
